...
Unfortunately I was unable to complete this section of the project. I was going to use kernel density to determine how the observations were clustered for families of soft coral.  I was interested in the clustering of families because this could be used to show resilience of certain families if they clustered in areas of higher temperature. It could also show preference of different families for temperatures and hint towards their sensitivity to change. 

```{r, results='hide'}

inter_GBR = GBR_97["PERIMETER"]
#my attempts to create a grid around the GBR area
#no matter what I tried it made a square grid around the whole bounding area instead of within the polygon


#sf way to create a grid, turned into a perfect sqaure grid

grid = st_make_grid(inter_GBR, what = "polygons")
plot(grid)

#did checks to understand what was going on with the objects
check =st_bbox(inter_GBR)


#tried to get the coordinates and use the old sp package to figure out the issue. No luck. 
trial = as.data.frame(st_coordinates(inter_GBR))
trial_sf = st_as_sf(inter_GBR)
class(inter_GBR)

trial

trial_sub = trial %>% select(X, Y)
...
grid = st_make_grid(inter_GBR, square = TRUE)

plot(grid)


#tried this as a last ditch effort, does intersect the squre grid, but doesn't actually make a grid of the area
#so interpolation wasn't going to be accurate or maybe even possible, I'm not sure. I ran out of time and patience. 
together = st_intersection(inter_GBR, grid)

plot(together)

new_grid = st_make_grid(together)
